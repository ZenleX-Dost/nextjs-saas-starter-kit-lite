...
export default function DefectLocalizationView({
  ...
  useEffect(() => {
    ...
    regions.forEach((region, idx) => {
      ...
      const coverageText = `${(region.coverage * 100).toFixed(1)}%`;
  ...
  const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (clickedRegionIdx !== -1) {
      ...
      if (onRegionClick) {
        onRegionClick(regions[clickedRegionIdx]);
      }
    } else {
            ...
            {regions.map((region, idx) => (
              ...
              >
                ...
                <div className="mt-2 space-y-1 text-xs text-gray-700">
                  <p>Coverage: <span className="font-medium">{(region.coverage * 100).toFixed(1)}%</span></p>
                  <p>Intensity: <span className="font-medium">{(region.intensity * 100).toFixed(1)}%</span></p>
